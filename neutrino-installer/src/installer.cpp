...
bool run_neupak(const char* target_root,
                const char* command,
    ...
    size_t len = 0;
    args[0] = '\0';
    const char* pieces[] = {"--root ", target_root, " ", command};
    for (const char* piece : pieces) {
        size_t piece_len = strlen(piece);
        if (len + piece_len + 1 > sizeof(args)) {
            userspace::write_line(console, "package installer arguments are too long");
            return false;
        }
        memcpy(args + len, piece, piece_len);
        len += piece_len;
        args[len] = '\0';
    }

    long pid = child(kNeupakPath, args, 0, nullptr);
    if (pid < 0) {
        userspace::write(console, "failed to launch neupak: ");
        userspace::write_line(console, error_label);
        return false;
    }
    long status = process_wait_child(static_cast<uint32_t>(pid), false);
    if (status < 0 || (static_cast<uint64_t>(status) & 0xffffu) != 0) {
        userspace::write(console, "neupak command failed: ");
        userspace::write_line(console, error_label);
        return false;
    }
    return true;
...
bool bootstrap_packages(const char* target_root, long console) {
    ...
    file_remove(target_repos);
    if (!copy_file_path(kOnlineReposPath, target_repos, console)) {
        userspace::write_line(console, "failed to restore online package repositories");
        return false;
    }
    return true;
}

bool provision_installed_credentials(const char* target_root, long console) {
    if (!ensure_installed_directory(target_root, "system", console)) {
        return false;
    }
    ...
    return copy_file_path("/system/users.ntd", destination, console);
}
...
bool write_installed_esp_config(const char* esp_name,
    ...
    bool ok =
        ...
        write_config_chunk(config_handle, " MOUNT=", "boot mount option", console) &&
        write_config_chunk(config_handle, esp_name, "boot mount device", console) &&
        write_config_chunk(config_handle, "\n\n", "entry separator", console) &&
        write_config_chunk(config_handle,
                           "/Neutrino (previous kernel)\n",
                           "recovery entry",
                           console) &&
        write_config_chunk(config_handle,
                           "    protocol: limine\n",
                           ...
                           console) &&
        write_config_chunk(config_handle,
                           "    path: boot():/boot/kernel.previous.elf\n",
                           "recovery kernel path",
                           console) &&
        write_config_chunk(config_handle,
                           "    cmdline: ROOT=",
                           "recovery cmdline",
                           console) &&
        write_config_chunk(config_handle, root_name, "recovery root device", console) &&
        write_config_chunk(config_handle, " MOUNT=", "recovery mount option", console) &&
        write_config_chunk(config_handle, esp_name, "recovery boot mount", console) &&
        write_config_chunk(config_handle, "\n", "newline", console);
...
bool finalize_cloned_target(const Device& dst, long console) {
    ...
    if (file_remove(user_store_path) != 0 ||
        !copy_file_path("/system/users.ntd", user_store_path, console)) {
