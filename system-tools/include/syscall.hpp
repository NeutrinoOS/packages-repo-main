...
static inline long machine_settings_get(const char* key, char* value = nullptr,
                                        size_t value_size = 0) {
    return key == nullptr || (value == nullptr && value_size != 0) ? -1 : raw_syscall3(
        SystemCall::MachineSettingsGet, reinterpret_cast<long>(key),
        reinterpret_cast<long>(value), static_cast<long>(value_size));
}
static inline long machine_settings_set(const char* key, const char* value) {
    return key == nullptr || value == nullptr ? -1 : raw_syscall2(
        SystemCall::MachineSettingsSet, reinterpret_cast<long>(key), reinterpret_cast<long>(value));
}
