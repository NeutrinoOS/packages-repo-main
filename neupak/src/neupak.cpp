...
bool set_target_path(char* out, size_t out_size, const char* suffix) {
    size_t len = 0;
    out[0] = '\0';
    return userspace::text::append_text(out, out_size, len, kSysrootPrefix) &&
           userspace::text::append_text(out, out_size, len, suffix);
}

bool valid_offline_root(const char* path) {
    ...
    while (*component != '\0') {
        ...
        size_t length = static_cast<size_t>(end - component);
        if (length == 0 ||
            (length == 1 && component[0] == '.') ||
            (length == 2 && component[0] == '.' && component[1] == '.') ||
            (length == 3 && component[0] == '.' && component[1] == '.' &&
             component[2] == '.')) {
            return false;
        }
...
bool configure_target_root(const char* root) {
    ...
    strlcpy(kSysrootPrefix, root, sizeof(kSysrootPrefix));
    size_t root_len = strlen(kSysrootPrefix);
    while (root_len > 1 && kSysrootPrefix[root_len - 1] == '/') {
        kSysrootPrefix[--root_len] = '\0';
    }
    long directory = directory_open(kSysrootPrefix);
    if (directory < 0) {
        print("neupak: offline root is not a mounted directory: ");
        print_line(kSysrootPrefix);
        return false;
    }
    directory_close(static_cast<uint32_t>(directory));
    return set_target_path(kConfigDir, sizeof(kConfigDir), "/config/neupak") &&
           set_target_path(kCacheDir, sizeof(kCacheDir), "/config/neupak/cache") &&
           set_target_path(kRepoCacheDir, sizeof(kRepoCacheDir), "/config/neupak/cache/repos") &&
           set_target_path(kPackageCacheDir, sizeof(kPackageCacheDir), "/config/neupak/cache/packages") &&
           set_target_path(kExtractDir, sizeof(kExtractDir), "/config/neupak/cache/extract") &&
           set_target_path(kManifestCachePath, sizeof(kManifestCachePath), "/config/neupak/cache/manifest.tmp") &&
           set_target_path(kReposPath, sizeof(kReposPath), "/config/neupak/repos.cfg") &&
           set_target_path(kInstalledPath, sizeof(kInstalledPath), "/config/neupak/install.db") &&
           set_target_path(kFilesPath, sizeof(kFilesPath), "/config/neupak/files.db") &&
           set_target_path(kLockPath, sizeof(kLockPath), "/config/neupak/db.lck");
}
...
bool append_boot_path(const char* root,
                      const char* logical_path,
                      char* out,
                      size_t out_size) {
    if (root == nullptr || logical_path == nullptr || logical_path[0] != '/') {
        return false;
    }
    size_t len = 0;
    out[0] = '\0';
    return userspace::text::append_text(out, out_size, len, root) &&
           userspace::text::append_text(out, out_size, len, logical_path);
}

bool is_neutrino_boot_mount(const char* root) {
    char kernel_path[kMaxPath];
    char config_path[kMaxPath];
    if (!append_boot_path(root, "/boot/kernel.elf", kernel_path, sizeof(kernel_path)) ||
        !append_boot_path(root, "/limine.conf", config_path, sizeof(config_path)) ||
        !file_exists(kernel_path)) {
        return false;
    }
    if (file_exists(config_path)) {
        return true;
    }
    return append_boot_path(root, "/LIMINE.CNF", config_path, sizeof(config_path)) &&
           file_exists(config_path);
}

bool partition_is_fat32(uint32_t handle) {
    ...
    if (descriptor_get_property(
            ...
            &geometry,
            sizeof(geometry)) != 0 ||
        geometry.sector_size < 512 || geometry.sector_size > 4096) {
        return false;
    }
    uint8_t sector[4096];
    if (descriptor_read(handle, sector, static_cast<size_t>(geometry.sector_size), 0) !=
        static_cast<long>(geometry.sector_size)) {
        return false;
    }
    return sector[510] == 0x55 && sector[511] == 0xAA &&
           sector[11] == 0x00 && sector[12] == 0x02 && sector[13] != 0 &&
           (sector[36] != 0 || sector[37] != 0 ||
            sector[38] != 0 || sector[39] != 0);
}

bool find_boot_root() {
    if (g_boot_root[0] != '\0' && is_neutrino_boot_mount(g_boot_root)) {
        return true;
    }
    g_boot_root[0] = '\0';
    (void)rescan_block_devices();

    constexpr uint32_t kDisk = static_cast<uint32_t>(descriptor_defs::Type::Disk);
    constexpr uint32_t kPartition =
        static_cast<uint32_t>(descriptor_defs::Type::Partition);
    for (uint64_t disk_index = 0;; ++disk_index) {
        long disk = descriptor_open(kDisk, 0, disk_index, 0);
        if (disk < 0) {
            break;
        }
        descriptor_defs::DiskInfo disk_info{};
        bool disk_ok = descriptor_get_property(
        ...
        for (uint32_t part_index = 0; part_index < disk_info.partition_count; ++part_index) {
            descriptor_defs::PartitionInfo info{};
            if (descriptor_read(static_cast<uint32_t>(disk),
                                &info,
                                sizeof(info),
                                static_cast<uint64_t>(part_index) * sizeof(info)) !=
                static_cast<long>(sizeof(info))) {
                continue;
            }
            char existing_root[kMaxPath] = "/";
            strlcpy(existing_root + 1, info.name, sizeof(existing_root) - 1);
            if (is_neutrino_boot_mount(existing_root)) {
                strlcpy(g_boot_root, existing_root, sizeof(g_boot_root));
                descriptor_close(static_cast<uint32_t>(disk));
                return true;
            }

            long partition = descriptor_open(
                kPartition, static_cast<uint64_t>(disk), part_index, 0);
            if (partition < 0) {
                continue;
            }
            bool fat32 = partition_is_fat32(static_cast<uint32_t>(partition));
            bool mounted = fat32 &&
                           mount_descriptor(static_cast<uint32_t>(partition), nullptr) == 0;
    ...
    print_line("neupak: unable to locate the installed Neutrino boot partition");
    return false;
}

bool destination_path(const char* strategy,
                      const char* logical_path,
                      char* out,
                      size_t out_size) {
    if (strcmp(strategy, "over_root") == 0) {
        return sysroot_path_for_logical(logical_path, out, out_size);
    }
    if (strcmp(strategy, "over_boot") == 0 && find_boot_root()) {
        return append_boot_path(g_boot_root, logical_path, out, out_size);
    }
    return false;
}
...
bool validate_package(const Package& pkg, bool require_hash) {
    ...
    if (strcmp(pkg.strategy, "over_root") != 0 &&
        strcmp(pkg.strategy, "over_boot") != 0) {
...
bool derive_package_url(const char* indexurl,
    ...
    size_t len = 0;
    out[0] = '\0';
    if (userspace::text::starts_with(indexurl, "file://")) {
...
bool copy_local_repository_file(const char* source, const char* output) {
    long input = file_open(source);
    if (input < 0) {
        return false;
    }
    long existing = file_open(output);
    if (existing >= 0) {
        file_close(static_cast<uint32_t>(existing));
        if (file_remove(output) < 0) {
            file_close(static_cast<uint32_t>(input));
            return false;
        }
    }
    long destination = file_create(output);
    if (destination < 0) {
        file_close(static_cast<uint32_t>(input));
        return false;
    }
    ...
    while (ok) {
        long got = file_read(
            static_cast<uint32_t>(input), g_copy_buffer, sizeof(g_copy_buffer));
        if (got < 0) {
            ok = false;
            break;
        }
        ...
        while (written < static_cast<size_t>(got)) {
            long result = file_write(
                static_cast<uint32_t>(destination),
                g_copy_buffer + written,
    ...
    if (ok && file_sync(static_cast<uint32_t>(destination)) != 0) {
        ok = false;
    }
    file_close(static_cast<uint32_t>(input));
    file_close(static_cast<uint32_t>(destination));
    if (!ok) {
        file_remove(output);
    }
    return ok;
...
bool run_download(const char* url, const char* output, bool quiet) {
    ...
    if (userspace::text::starts_with(url, "file://")) {
        const char* source = url + strlen("file://");
        if (source[0] == '\0') {
            print_line("neupak: empty file repository path");
            return false;
        }
        return copy_local_repository_file(source, output);
    }
    char args[900];
    size_t len = 0;
    args[0] = '\0';
...
bool load_installed(InstalledDb& db) {
    ...
    while (line < text + len) {
        ...
        if (parse_section_header(cleaned, section, sizeof(section))) {
            ...
            } else if (key != nullptr && strcmp(key, "strategy") == 0) {
                if (!parse_quoted(value, current->strategy, sizeof(current->strategy))) {
                    unmap(text, kMaxText + 1);
                    return false;
                }
    ...
    unmap(text, kMaxText + 1);
    for (size_t i = 0; i < db.count; ++i) {
        if (db.packages[i].strategy[0] == '\0') {
            strlcpy(db.packages[i].strategy,
                    "over_root",
                    sizeof(db.packages[i].strategy));
        }
    }
    return true;
}
...
bool write_installed_db(const InstalledDb& db) {
    ...
    for (size_t i = 0; i < db.count; ++i) {
        if (!userspace::text::append_char(text, kMaxText, len, '[') ||
            ...
            !userspace::text::append_text(text, kMaxText, len, "\"\nstrategy = \"") ||
            !userspace::text::append_text(text, kMaxText, len, db.packages[i].strategy) ||
            !userspace::text::append_text(text, kMaxText, len, "\"\n\n")) {
            unmap(text, kMaxText);
            return false;
        }
...
bool plan_zip(const char* path, ZipPlan& plan) {
    ...
    file_close(static_cast<uint32_t>(file));
    // A dependency-only meta-package legitimately has no payload entries.
    // The archive still passed structural validation and its manifest is
    // validated separately before the transaction is committed.
    return true;
}
...
bool check_conflicts(const ZipPlan& plan,
                     const FilesDb& files,
                     const Package& pkg,
                     bool force) {
    for (size_t i = 0; i < plan.count; ++i) {
        const char* owner = nullptr;
        if (owned_by_other(files, pkg.name, plan.entries[i].final_path, owner)) {
            if (!force) {
        ...
        if (owner == nullptr) {
            ...
            if (!destination_path(pkg.strategy,
            ...
            if (existing >= 0) {
                ...
                bool preinstalled_boot_file = strcmp(pkg.strategy, "over_boot") == 0;
                if (!force && !preinstalled_library && !preinstalled_boot_file &&
...
bool copy_file_replace(const char* source, const char* dest) {
    ...
    if (ok && file_sync(static_cast<uint32_t>(out)) != 0) {
        ok = false;
    }
    file_close(static_cast<uint32_t>(in));
    file_close(static_cast<uint32_t>(out));
...
bool copy_temp_into_place(const Package& pkg, const ZipPlan& plan) {
    for (size_t i = 0; i < plan.count; ++i) {
        char temp_path[kMaxPath];
        char dest_path[kMaxPath];
        if (!temp_payload_path(pkg.name, plan.entries[i].final_path, temp_path, sizeof(temp_path)) ||
            !destination_path(pkg.strategy,
                              plan.entries[i].final_path,
                              dest_path,
                              sizeof(dest_path))) {
            return false;
        }
        if (strcmp(pkg.strategy, "over_boot") == 0) {
            char backup_path[kMaxPath];
            if (!append_boot_path(g_boot_root,
                                  "/boot/kernel.previous.elf",
                                  backup_path,
                                  sizeof(backup_path)) ||
                !copy_file_replace(dest_path, backup_path) || !neutrino_sync()) {
                print_line("neupak: unable to preserve the previous kernel");
                return false;
            }
            if (!copy_file_replace(temp_path, dest_path)) {
                print_line("neupak: kernel update failed; restoring the previous kernel");
                (void)copy_file_replace(backup_path, dest_path);
                (void)neutrino_sync();
                return false;
            }
        } else if (!copy_file_replace(temp_path, dest_path)) {
            return false;
        }
    }
    return true;
...
bool update_db_for_package(InstalledDb& installed,
    ...
    strlcpy(installed_pkg->strategy, pkg.strategy, sizeof(installed_pkg->strategy));
...
bool uninstall_package(const char* name) {
    ...
    InstalledPackage* installed_pkg = find_installed(g_installed, name);
    if (installed_pkg != nullptr &&
        strcmp(installed_pkg->strategy, "over_boot") == 0) {
        print_line("neupak: boot packages cannot be uninstalled; install a replacement instead");
        return false;
    }
...
bool install_zip_package(const char* zip_path, const Package& pkg, bool force) {
    ...
    if (strcmp(pkg.strategy, "over_boot") == 0 &&
        (g_plan.count != 1 ||
         strcmp(g_plan.entries[0].final_path, "/boot/kernel.elf") != 0)) {
        print_line("neupak: boot packages may only contain boot/kernel.elf");
        return false;
    }
    ...
    if (!load_installed(g_installed)) {
        ...
        ok = false;
    } else if (!check_conflicts(g_plan, g_files, pkg, force)) {
        ok = false;
    } else if (!copy_temp_into_place(pkg, g_plan)) {
...
void usage() {
    print_line("usage: neupak [--root <mounted-root>] update-index");
    print_line("       neupak [--root <mounted-root>] search <query>");
    print_line("       neupak [--root <mounted-root>] install [--force-overwrite] <package>");
    print_line("       neupak [--root <mounted-root>] install-local [--force-overwrite] <zip-path>");
    print_line("       neupak [--root <mounted-root>] files <package>");
    print_line("       neupak [--root <mounted-root>] uninstall <package>");
}
...
int main(uint64_t arg_ptr, uint64_t) {
    ...
    char target_root_storage[kMaxPath];
    const char* target_root = nullptr;
    if (strcmp(args.words[0], "--root") == 0) {
        if (args.count < 3) {
            usage();
            return 1;
        }
        strlcpy(target_root_storage, args.words[1], sizeof(target_root_storage));
        target_root = target_root_storage;
        for (size_t i = 2; i < args.count; ++i) {
            strlcpy(args.words[i - 2], args.words[i], sizeof(args.words[i - 2]));
        }
        args.count -= 2;
    }
    if (!configure_target_root(target_root)) {
        return 1;
    }
